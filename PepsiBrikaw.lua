local library = {
	Version = "0.33",
	WorkspaceName = "Luxury Hub Pack 1",
	flags = {},
	signals = {},
	objects = {},
	elements = {},
				...
				if getsynasset then
					...
					elseif #image > 14 and string.sub(image, 1, 14) == "synasseturl://" then
						local x, e = pcall(function()
							...
							local fold = isfolder("./Luxury Hub Pack 1")
							if not fold then
								makefolder("./Luxury Hub Pack 1")
							end
							fold = isfolder("./Luxury Hub Pack 1/Themes")
							if not fold then
								makefolder("./Luxury Hub Pack 1/Themes")
							end
							fold = isfolder("./Luxury Hub Pack 1/Themes/SynapseAssetsCache")
							if not fold then
								makefolder("./Luxury Hub Pack 1 Themes/SynapseAssetsCache")
							end
							if not fold or not isfile("./Luxury Hub Pack 1/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
								local res = game:HttpGet(string.sub(image, 15))
								if res ~= nil then
									writefile("./Luxury Hub Pack 1/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
								end
							end
							return getsynasset(readfile("./Luxury Hub Pack 1/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
						end)
						if x and e ~= nil then
							return e
						end
...
function library:CreateWindow(options, ...)
	...
	if windowName and #windowName > 0 and library.WorkspaceName == "Luxury Hub Pack 1" then
		library.WorkspaceName = convertfilename(windowName, "Luxury Hub Pack 1")
	end
		...
		function tabFunctions:CreateSection(options, ...)
			...
			if isfolder and makefolder and listfiles and readfile and writefile then
				function sectionFunctions:AddPersistence(options, ...)
					...
					local realDropdownHolderList = Instance_new("UIListLayout")
					local dropdownEnabled = false
					if not isfolder("./Luxury Hub Pack 1") then
						makefolder("./Luxury Hub Pack 1")
					end
					local common_string = "./Luxury Hub Pack 1/" .. tostring(custom_workspace or library.WorkspaceName)
					local function resolvelist(nofold)
						if custom_workspace ~= options.Workspace then
							custom_workspace = options.Workspace
							common_string = "./Luxury Hub Pack 1/" .. tostring(custom_workspace or library.WorkspaceName)
						end
	...
	function windowFunctions:CreateDesigner(options, ...)
		...
		local daaata = {{"AddTextbox", "__Designer.Textbox.ImageAssetID", backgroundsection, {
			Name = "Image Asset ID",
			Placeholder = "rbxassetid://12018435108",
			Flag = "__Designer.Background.ImageAssetID",
			Value = "rbxassetid://12018435108",
			...
			Workspace = "Luxury Hub Pack 1 Themes",
			Flags = flags,
			Suffix = "Theme",
			Desginer = true
